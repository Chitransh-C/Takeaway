...
async function sendOrderStatusUpdate(orderId, status, customerPhone) {
    console.log(`Sending WhatsApp message to ${customerPhone}: Order ${orderId} status updated to ${status}`);
    ...
    if (!accountSid || !authToken || !TWILIO_PHONE) {
        console.error("❌ Twilio credentials are missing or incorrect.");
        alert("Twilio credentials are not configured correctly.");
        return;
    }

    const message = `Your order (${orderId}) status has been updated to: ${status}. Thank you for ordering!`;
...
async function sendOrderConfirmation(orderId, customerPhone) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.error("❌ Failed to send WhatsApp confirmation message:", result);
